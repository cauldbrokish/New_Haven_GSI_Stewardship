...
# 2 Link recent inspections to inlets
```{r}
...
inlet_recent_inspection <- 
  inlet |> 
  left_join(
    recent_inspections |> select(GlobalID, FACILITYID, DEBRIS, VEGETATION, OIL, Condition, islegit)
    , by = c('FACILITYID' = 'FACILITYID') # not perfect like Dawn said
    )
