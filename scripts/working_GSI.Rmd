...
inlet <- 
  ...
  group_by(districts) |> # FIXME this is making the area by district, is that what you wnat here?
  mutate(
...
inspection_long <- 
  inspection |> 
  left_join(
    inlet |> select(-c(CreationDate, edit_date, Creator, Editor))
...
inspection_long |>
  ...
  ggplot(aes(edit_month.x, Condition, group = ASSETGUID, fill = Assigned)) + # edit_month is no longer a date, its a number
  geom_col() +
